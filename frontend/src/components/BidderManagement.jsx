...
export default function BidderManagement({ tenderId, criteria, bidders, setBidders, onEvaluationComplete }) {
  ...
  const PROGRESS_STEPS = [
    "Parsing bidder document structure...",
    ...
    "Finalizing bidder evaluation profile..."
  ];
  ...
  const handleEvaluate = async () => {
    ...
    try {
      for (let i = 0; i < bidders.length; i++) {
        ...
        setEvalProgress(`Evaluating [${i + 1}/${bidders.length}]: ${bidder.name}`);
        
        let filesToUpload = [];
        bidder.contents.forEach((c, idx) => {
          if (c.type === 'paste') {
            const blob = new Blob([c.data], { type: 'text/plain' });
            filesToUpload.push(new File([blob], `${bidder.name}_text_${idx+1}.txt`, { type: 'text/plain' }));
          } else {
            filesToUpload.push(c.data);
          }
        });
        
        const evalData = await evaluateBidder(tenderId, bidder.name, filesToUpload);
        results[bidder.name] = evalData.result;
      }
