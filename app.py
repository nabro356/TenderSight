...
Streamlit App — Flow: Upload Tender → Add Bidders → Evaluate → Report
"""
...
def init_state():
    defaults = {"step": 1, "api_key": NVIDIA_API_KEY, "tender_text": "", "criteria": [],
                "bidders": {}, "evaluations": {}, "chat_history": [], "bidder_counter": 0}
...
def _clean_engine_name(engine):
    """Show user-friendly engine name without exposing model details."""
    if "gemini" in str(engine).lower() or "vlm" in str(engine).lower() or "vision" in str(engine).lower():
        return "Vision OCR"
    if "pypdf" in str(engine).lower():
        return "PDF Parser"
    if "plain" in str(engine).lower():
        return "Text"
    return "Auto"
...
with st.sidebar:
    ...
    st.info("API Key is securely loaded from environment variables (.env).")
    st.divider()
...
if st.session_state.step == 1:
    st.markdown("## 📄 Step 1: Upload Tender Document")

    tab_file, tab_text = st.tabs(["📁 Upload File", "📝 Paste Text"])

    with tab_file:
        uploaded = st.file_uploader("Upload Tender (.txt, .pdf)", type=["txt", "pdf"], key="tu")
        if uploaded and not st.session_state.tender_text:
            if st.button("📤 Submit Tender Document", use_container_width=True, type="primary", key="submit_file"):
                doc_status = st.empty()
                with st.spinner("Processing document..."):
                    r = ingest_document(
                        uploaded.read(), uploaded.name,
                        api_key=st.session_state.api_key,
                        progress_callback=lambda m: doc_status.markdown(f"**{m}**"),
                    )
                    st.session_state.tender_text = r["raw_text"]
                    eng = _clean_engine_name(r.get('ocr_engine', ''))
                    conf = r.get('confidence', 0)
                    doc_status.markdown(f"**✅ Document processed** — {eng} · {conf:.0%} confidence · {len(r['raw_text'])} chars")
                    time.sleep(1.5)
                st.rerun()

    with tab_text:
        pasted = st.text_area("Paste tender document text below:", height=250, key="paste_tender",
                              placeholder="Paste the full tender document text here...")
        if pasted and not st.session_state.tender_text:
            if st.button("📤 Submit Text", use_container_width=True, type="primary", key="submit_text"):
                st.session_state.tender_text = pasted.strip()
                st.rerun()

    if st.session_state.tender_text:
        with st.expander("📖 Document Preview", expanded=False):
            st.text(st.session_state.tender_text[:3000])
        ...
        st.info("💡 Upload a tender document to begin evaluation.")
...
# ═══════════════════════════════════════════════
    ...
    with st.expander(f"📋 {len(st.session_state.criteria)} Eligibility Criteria Extracted", expanded=False):
        for c in st.session_state.criteria:
            ...
            color = type_colors.get(ctype, "#64748b")
            icon = type_icons.get(ctype, "📌")
            mandatory = "✅ Mandatory" if c.get("mandatory", True) else "Optional"
            sub_html = ""
            for sc in c.get("sub_conditions", []):
                parts = [f'<code>{sc.get("parameter") or ""}</code>' if sc.get("parameter") else "",
                         sc.get("operator") or "",
                         f'<strong>{sc.get("threshold") or ""}</strong>' if sc.get("threshold") else "",
                         sc.get("unit") or ""]
                desc = " ".join(p for p in parts if p)
                if desc.strip():
                    sub_html += f'<div style="font-size:.8rem;color:#475569;padding:2px 0">• {desc}</div>'
            st.markdown(f"""<div class="clean-card" style="border-left:4px solid {color}">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem">
                    <div><span style="font-size:1.1rem">{icon}</span> <span style="font-weight:700;color:#1e293b">{cid}</span>
                    <span class="badge-sm" style="background:{color}20;color:{color}">{ctype}</span></div>
                    <div style="font-size:.75rem;color:#94a3b8">{mandatory}</div>
                </div>
                <div style="color:#334155;font-size:.88rem;line-height:1.5">{c.get("text", "")}</div>
                {f'<div style="margin-top:.5rem;padding-top:.5rem;border-top:1px solid #f1f5f9">{sub_html}</div>' if sub_html else ""}
    ...
    st.markdown("Enter a bidder name, upload their document, and click **Submit** to add.")

    # Use counter to create unique keys so fields reset after each add
    bc = st.session_state.bidder_counter

    bname = st.text_input("Bidder Name", placeholder="e.g. Bharat Constructions", key=f"bname_{bc}")

    btab_file, btab_text = st.tabs(["📁 Upload File", "📝 Paste Text"])

    with btab_file:
        bfile = st.file_uploader("Upload bidder doc (.txt, .pdf)", type=["txt", "pdf"], key=f"bfile_{bc}", label_visibility="collapsed")
        if bfile and bname:
            if st.button("📤 Submit Bidder", use_container_width=True, type="secondary", key=f"bsub_file_{bc}"):
                bidder_status = st.empty()
                with st.spinner(f"Processing {bname}'s document..."):
                    r = ingest_document(
                        bfile.read(), bfile.name,
                        api_key=st.session_state.api_key,
                        progress_callback=lambda m: bidder_status.markdown(f"**{m}**"),
                    )
                    eng = _clean_engine_name(r.get('ocr_engine', ''))
                    conf = r.get('confidence', 0)
                    bidder_status.markdown(f"**✅ {bname}** — {eng} · {conf:.0%} · {len(r['raw_text'])} chars")
                    st.session_state.bidders[bname] = r["raw_text"]
                    st.session_state.bidder_counter += 1
                time.sleep(1)
                st.rerun()
        elif bfile and not bname:
            st.warning("⚠️ Enter a bidder name above first")

    with btab_text:
        btext = st.text_area("Paste bidder submission text:", height=200, key=f"btext_{bc}",
                             placeholder="Paste the bidder's submission document text here...")
        if btext and bname:
            if st.button("📤 Submit Bidder", use_container_width=True, type="secondary", key=f"bsub_text_{bc}"):
                st.session_state.bidders[bname] = btext.strip()
                st.session_state.bidder_counter += 1
                st.rerun()
        elif btext and not bname:
            st.warning("⚠️ Enter a bidder name above first")
    ...
    if st.session_state.bidders:
        ...
        for i, name in enumerate(list(st.session_state.bidders.keys())):
            c1, c2 = st.columns([5, 1])
            with c1:
                st.markdown(f"**{i+1}.** 🏢 {name}")
            with c2:
                if st.button("🗑️", key=f"del_{i}"):
                    del st.session_state.bidders[name]
                    st.rerun()
        ...
        if not st.session_state.evaluations:
            if st.button("🚀 Evaluate All Bidders", use_container_width=True, type="primary"):
                ...
                for idx, name in enumerate(names):
                    ...
                    try:
                        ...
                        evaluation = evaluate_bidder(
                            ...
                            api_key=st.session_state.api_key,
                            progress_callback=lambda m: detail_text.markdown(m),
                        )
                        ...
                        st.session_state.evaluations[name] = {
                            "bidder_name": name, "overall_status": "MANUAL_REVIEW",
                            "verdicts": [], "eligible_count": 0,
                ...
                status_text.markdown("**✅ All evaluations complete!**")
                detail_text.empty()
                time.sleep(1)
...
# ═══════════════════════════════════════════════
    ...
    try:
        ...
        if bidder_names and crit_ids:
            # Grouped confidence bar chart
            fig, ax = plt.subplots(figsize=(max(10, len(crit_ids) * 1.5), 5))
            fig.patch.set_facecolor("#ffffff")
            ax.set_facecolor("#f8fafc")
            x = np.arange(len(crit_ids))
            width = 0.8 / max(len(bidder_names), 1)
            ...
            for i, name in enumerate(bidder_names):
                ...
                bars = ax.bar(x + i * width - 0.4 + width / 2, vals, width * 0.9,
                              label=name[:20], color=colors[i % len(colors)], edgecolor="white", linewidth=0.5, alpha=0.85)
                for bar, val in zip(bars, vals):
                    if val > 0:
                        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.02,
                                f"{val:.0%}", ha="center", va="bottom", fontsize=7, color="#475569", fontweight="bold")

            ax.set_xticks(x)
            ax.set_xticklabels(crit_ids, fontsize=9, fontweight="bold", color="#334155")
            ax.set_ylim(0, 1.15)
            ax.set_ylabel("Confidence", fontsize=10, color="#64748b")
            ...
            for i, name in enumerate(bidder_names):
                e = evs[name]
                total = max(e.get("eligible_count", 0) + e.get("not_eligible_count", 0) + e.get("manual_review_count", 0), 1)
                elig_pct = e.get("eligible_count", 0) / total
                fail_pct = e.get("not_eligible_count", 0) / total
                rev_pct = e.get("manual_review_count", 0) / total
            ...
            ax2.set_yticks(range(len(bidder_names)))
            ax2.set_yticklabels([n[:25] for n in bidder_names], fontsize=10, color="#334155")
            ax2.set_xlim(0, 1)
            ax2.set_xlabel("Proportion", fontsize=10, color="#64748b")
            ...
            plt.tight_layout()
            buf2 = io.BytesIO()
            fig2.savefig(buf2, format="png", dpi=150, bbox_inches="tight", facecolor="#fff")
            buf2.seek(0)
            plt.close(fig2)
    ...
    for name, ev in evs.items():
        ...
        with st.expander(f"{icon} {name} — {overall.replace('_', ' ')}", expanded=(overall != "ELIGIBLE")):
            if ev.get("error"):
                st.error(f"Error: {ev['error']}")
                continue
            for v in ev.get("verdicts", []):
                s = v.get("status", "MANUAL_REVIEW")
    ...
    for name, ev in evs.items():
        ...
        o = ev["overall_status"]
        cells.append("✅" if o == "ELIGIBLE" else "❌" if o == "NOT_ELIGIBLE" else "⚠️")
    ...
    with c2:
        data = {"criteria": criteria, "evaluations": evs, "timestamp": datetime.now().isoformat()}
    ...
    if not st.session_state.chat_history:
        suggestions = []
        for name, ev in evs.items():
            if ev["overall_status"] == "MANUAL_REVIEW":
                suggestions.append(f"Why was {name} flagged for manual review?")
            elif ev["overall_status"] == "NOT_ELIGIBLE":
                suggestions.append(f"Why was {name} not eligible?")
        if len(evs) > 1:
            suggestions.append("Compare all bidders and rank them")
        if suggestions:
