...
class OrchestratorAgent(BaseAgent):
    ...
    def process_tender_bundle(
        ...
        """
        Process a tender bundle made up of one or more source documents.

        files: [{"filename": str, "bytes": bytes}, ...]
        """
        if not files:
            raise ValueError("No tender files were provided")
        ...
        for idx, file_info in enumerate(files, 1):
            ...
            if progress_callback:
                progress_callback(f"📄 Ingesting tender source {idx}/{len(files)}: {filename}")
            ...
            bundle_documents.append(
                {
                    "name": filename,
                    "pages": ingestion.get("pages", 0),
                    "ocr_engine": ingestion.get("ocr_engine"),
                    "confidence": ingestion.get("confidence", 0.0),
                    "status": "PARSED" if ingestion.get("raw_text") else "FAILED",
                }
            )
            ...
            total_confidence += ingestion.get("confidence", 0.0)

        combined_text = "\n\n".join(combined_parts).strip()
        if not combined_text:
            raise ValueError("Could not extract text from the tender bundle")

        if progress_callback:
            progress_callback("🔍 Extracting eligibility criteria from tender bundle...")
        ...
        average_confidence = total_confidence / max(len(files), 1)

        return {
            "raw_text": combined_text,
            "criteria": criteria,
            "ocr_engine": "bundle_mixed",
            "confidence": average_confidence,
            "pages": sum(doc.get("pages", 0) for doc in bundle_documents),
    ...
    def evaluate_single_bidder(
        ...
        if not security.get("safe", True):
            return {
                ...
                "error": f"Security threat detected: {security.get('threats_detected', [])}",
